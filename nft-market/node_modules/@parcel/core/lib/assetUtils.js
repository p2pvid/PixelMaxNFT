...
function createAssetIdFromOptions(options) {
  ...
  let queryString = options.query ? JSON.stringify((0, _utils().objectSortedEntries)(options.query)) : '';
  return (0, _hash().hashString)(idBase + options.type + options.env.id + uniqueKey + ':' + ((_options$pipeline = options.pipeline) !== null && _options$pipeline !== void 0 ? _options$pipeline : '') + ':' + queryString);
}
...
async function getConfig(asset, filePaths, options) {
  ...
  let conf = await (0, _utils().loadConfig)(asset.options.inputFS, asset.value.filePath, filePaths, asset.options.projectRoot, parse == null ? null : {
    parse
  });
      ...
      case 'file':
        {
          ...
          if (fileHash == null) {
            fileHash = (0, _utils().hashFile)(options.inputFS, invalidation.filePath);
